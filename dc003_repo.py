# dc003_repo.py — Supabase client version (optional design_id, public schema)
...
TABLE = "dc003_calcs"   # bare; schema is set via .schema("public")
ENTITY = "dc003"

print("[dc003_repo] loaded (Supabase) from:", __file__)
...
def _is_undefined_column(exc: Exception) -> bool:
    """
    Detect PostgREST 'undefined_column' (SQLSTATE 42703) from supabase-py.
    """
    msg = str(exc).lower()
...
def _probe_design_id() -> bool:
    ...
    try:
        ...
        if _is_undefined_column(e):
            ...
            # On unrelated errors, default to True so we don't hide real issues.
            _HAS_DESIGN_ID = True
    return _HAS_DESIGN_ID
...
def create_dc003_calc(
    ...
    nm = (name or "DC003").strip() or "DC003"
...
def update_dc003_calc(
    ...
    *,
    ...
    sets: Dict[str, Any] = {}
    if name is not None:
        sets["name"] = (name or "DC003").strip() or "DC003"
