# dc005a_repo.py — Supabase client version (optional design_id, public schema)
...
TABLE  = "dc005a_calcs"   # bare table; we set schema("public") via client
ENTITY = "dc005a"

print("[dc005a_repo] loaded (Supabase) from:", __file__)
...
# Read (user-scoped)
# -------------------------------------------------------------------
def list_dc005a_calcs(user_id: str, limit: int = 200) -> List[Tuple[str, str, Any, Any]]:
    resp = (
        _tbl()
...
def get_dc005a_calc(calc_id: str, user_id: str) -> Optional[Dict[str, Any]]:
    resp = (
        _tbl()
    ...
    # Compose the shape the app expects (data merged + _meta)
    data = r.get("data") or {}
...
def update_dc005a_calc(
    ...
    *,
    ...
    if name is not None:
        sets["name"] = (name or "DC005A").strip() or "DC005A"
    if data is not None:
        sets["data"] = data
    ...
    # Fetch old name for audit fallback (if name not provided)
...
def delete_dc005a_calc(calc_id: str, user_id: str) -> bool:
    # prefetch name BEFORE delete so audit log shows it
    name_before = None
    try:
        pre = _tbl().select("name").eq("id", calc_id).eq("user_id", user_id).limit(1).execute()
        pre_rec = _first_or_none(pre)
