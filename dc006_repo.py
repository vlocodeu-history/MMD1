# dc006_repo.py — Supabase client version (optional design_id, public schema)
...
try:
    ...
    try:
        ...
        def _audit_log(*args, **kwargs):  # no-op if audit not available
            return

TABLE  = "dc006_calcs"   # bare table; we use schema("public") via client
ENTITY = "dc006"

print("[dc006_repo] loaded (Supabase) from:", __file__)
...
# Read (user-scoped)
# -------------------------------------------------------------------
def list_dc006_calcs(user_id: str, limit: int = 200) -> List[Tuple[str, str, Any, Any]]:
    resp = (
        _tbl()
...
def get_dc006_calc(calc_id: str, user_id: str) -> Optional[Dict[str, Any]]:
    # We include design_id in selection; PostgREST will omit it if column doesn't exist
    resp = (
        _tbl()
...
def update_dc006_calc(
    ...
    *,
    ...
    if name is not None:
        sets["name"] = (name or "DC006").strip() or "DC006"
    if data is not None:
        sets["data"] = data
