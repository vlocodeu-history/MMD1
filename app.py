...
if "boot_done" not in st.session_state:
    # DB check once (soft: only if secrets + psycopg2 are present)
    try:
        has_pg_secrets = False
        try:
            _sec = st.secrets.get("postgres", None)  # safe on Streamlit 1.50
            has_pg_secrets = bool(
                _sec
                and _sec.get("host")
                and _sec.get("user")
                and _sec.get("password")
            )
        except Exception:
            has_pg_secrets = False
        ...
        if has_pg_secrets and psy_ok:
            ...
            if not has_pg_secrets:
                print("DB check skipped: st.secrets['postgres'] not configured.")
            if has_pg_secrets and not psy_ok:
                print("DB check skipped: psycopg2 not installed. Add psycopg2-binary to requirements.")
    except Exception as e:
        # Do not crash the app during bootstrap
        print("DB bootstrap guard caught:", e)
