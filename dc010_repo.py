# dc010_repo.py — Supabase client version (optional design_id, public schema)
...
TABLE  = "dc010_calcs"  # bare table name; we call .schema("public")
ENTITY = "dc010"

print("[dc010_repo] loaded (Supabase) from:", __file__)
...
# List (user-scoped)
# -------------------------------------------------------------------
def list_dc010_calcs(user_id: str, limit: int = 200) -> List[Tuple[str, str, Any, Any]]:
    resp = (
        _tbl()
...
# Get one (user-scoped)
# -------------------------------------------------------------------
def get_dc010_calc(calc_id: str, user_id: str) -> Optional[Dict[str, Any]]:
...
def update_dc010_calc(
    ...
    *,
    ...
    if name is not None:
        sets["name"] = (name or "DC010").strip() or "DC010"
    if data is not None:
        sets["data"] = data
