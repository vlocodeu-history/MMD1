# dc004_repo.py — psycopg2-safe JSONB writes + optional design_id + public schema
...
def _insert_stmt_fallback(with_design: bool):
    # accepts serialized JSON via :data_text, cast to jsonb
    if with_design:
        return text(f"""
            INSERT INTO {TABLE} (user_id, design_id, name, data)
...
def create_dc004_calc(
    ...
    with connect() as conn:
        ...
        try:
            ...
            # Fallback: serialize to text then cast ::jsonb
...
def delete_dc004_calc(calc_id: str, user_id: str) -> bool:
    with connect() as conn:
        # read the name BEFORE deleting for audit trail
        try:
