# dc012_repo.py — psycopg2-safe JSONB writes + optional design_id + public schema
...
def _insert_stmt_fallback(with_design: bool):
    # Fallback when adapter won't bind dicts: serialize -> ::jsonb
    if with_design:
        return text(f"""
            INSERT INTO {TABLE} (user_id, design_id, name, data)
...
def update_dc012_calc(
    ...
    *,
    ...
    with connect() as conn:
        ...
        # Fetch old name for meaningful audit
        try:
        ...
        # Try JSONB bind, then fallback to ::jsonb cast
        try:
