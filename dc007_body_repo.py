# dc007_body_repo.py — Supabase client version (optional design_id, public schema)
...
TABLE  = "dc007_body_calcs"  # bare table name; we use schema("public")
ENTITY = "dc007_body"

print("[dc007_body_repo] loaded (Supabase) from:", __file__)
...
def _probe_design_id() -> bool:
    ...
    try:
        ...
        if _is_undefined_column(e):
            ...
            # On unrelated errors, assume True so real issues surface instead of being silently dropped.
            _HAS_DESIGN_ID = True
    return _HAS_DESIGN_ID
...
# Read (user-scoped)
# -------------------------------------------------------------------
def list_dc007_body_calcs(user_id: str, limit: int = 200) -> List[Tuple[str, str, Any, Any]]:
    resp = (
        _tbl()
...
def get_dc007_body_calc(calc_id: str, user_id: str) -> Optional[Dict[str, Any]]:
...
def update_dc007_body_calc(
    ...
    *,
    ...
    if name is not None:
        sets["name"] = (name or "DC007-Body").strip() or "DC007-Body"
    if data is not None:
        sets["data"] = data
