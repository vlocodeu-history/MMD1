# dc008_repo.py — Supabase client version (optional design_id, public schema)
...
TABLE  = "dc008_calcs"  # bare table name; we call .schema("public")
ENTITY = "dc008"

print("[dc008_repo] loaded (Supabase) from:", __file__)
...
def _is_undefined_column(exc: Exception) -> bool:
    """Detect PostgREST 'undefined column' (SQLSTATE 42703) from supabase-py error text."""
    msg = str(exc).lower()
...
def _probe_design_id() -> bool:
    ...
    try:
        ...
        if _is_undefined_column(e):
            ...
            # On unrelated errors, assume True so real issues surface rather than being hidden.
            _HAS_DESIGN_ID = True
    return _HAS_DESIGN_ID
...
# List (user-scoped)
# -------------------------------------------------------------------
def list_dc008_calcs(user_id: str, limit: int = 200) -> List[Tuple[str, str, Any, Any]]:
    resp = (
        _tbl()
...
# Get one (user-scoped)
# -------------------------------------------------------------------
def get_dc008_calc(calc_id: str, user_id: str) -> Optional[Dict[str, Any]]:
...
def update_dc008_calc(
    ...
    *,
    ...
    if name is not None:
        sets["name"] = (name or "DC008").strip() or "DC008"
    if data is not None:
        sets["data"] = data
